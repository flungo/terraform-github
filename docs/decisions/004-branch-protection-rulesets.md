...
- **Guard against classic branch protection** — the module reads the repo's classic
  ...
  The data source exposes only each rule's `pattern`, so a CI follow-on job
  (`surface-classic-protection`) reads the reusable workflow's plan artifact and fetches
  the full classic settings (GraphQL) when the guard fails, feeding the
  compare-before-remove migration in
