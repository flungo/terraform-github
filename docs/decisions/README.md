...
| [010](010-terraform-flag-means-terraform-standards.md) | The `terraform` flag means "follows Fabrizio's Terraform standards" | Accepted | Only two of five `terraform = true` repos actually used the `flungo/github-workflows` Terraform jobs, so the flag's read as "holds Terraform config" was wrong: that is a different property and the fleet proved they diverge. The flag now asserts the repo follows Fabrizio's Terraform standards — those jobs, under the conventional job and secret names — and both effects follow from that: the HCP token they read is attached, and the `terraform / terraform` check they report is required. `required_status_checks` becomes additional-only, and a new `excluded_status_checks` drops an implied context where a repo follows the standards but cannot report the check (`stalwart.flungo.net`), generalising to any future flag/check conflict. Resolves ADR-006's deferral. |
| [011](011-strict-required-status-checks.md) | Require branches to be up to date before merging | Accepted | A required check proves CI passed on the pull request's head, not on anything like what will land — two pull requests can each be green against an older `main` and break it together. `strict_required_status_checks_policy = true` is encoded in `modules/branch-protection` rather than exposed: there is no coherent position that wants a required check but not its strictness, and a second "strict" input beside the existing `strict` (which removes the admin bypass) would misread both. The block it lives in is emitted only where contexts are supplied, so it is absent entirely where none are required and arrives with each repository's CI adoption rather than needing its own rollout. Merge queue — the richer answer — is gated on *organisation* ownership rather than plan tier, so no upgrade reaches a personal account; its only added value here is throughput, which is worth nothing at one-pull-request-at-a-time volume. Refines ADR-004. |
| [012](012-markdown-flag-means-markdown-standards.md) | The `markdown` flag means "follows Fabrizio's Markdown standards" | Accepted | `LYCHEE_GITHUB_TOKEN` was attached to every managed repo since ADR-005, but only the external URL sweep reads it and four of nine repos run neither Markdown workflow — so the credential sat unread on nearly half the fleet, while the four that do run them gated nothing on the result. A `markdown` flag now asserts the repo follows Fabrizio's Markdown standards — the `flungo/github-workflows` Markdown workflows under the conventional caller job names `markdown-lint` and `markdown-links` — and both effects follow: the lychee token those jobs read is attached, and the `markdown-lint / lint` and `markdown-links / internal` checks they report are required. The sweep's own `markdown-links / external` is deliberately not required: it self-skips on `pull_request` so an outage cannot block a merge. First reuse of ADR-010's pattern for a second standard, which is what that ADR's framing was for. Reviewing the naming requirement produced `github-workflows`' own ADR-010 (a caller's job id is the reusable workflow's filename), which is what makes the caller half of a context predictable enough to hardcode; the four existing adopters were realigned to it first. |
