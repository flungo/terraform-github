...
| [`modules/repository-secrets`](../../modules/repository-secrets) | Shared Actions secrets, each attached where its standards flag is set | [`secrets.md`](secrets.md) |
...
| `markdown` | `bool` | `true` | The repo **follows Fabrizio's Markdown standards** — it uses the `flungo/github-workflows` Markdown workflows, under the conventional caller job names (`markdown-lint` and `markdown-links`, per [`github-workflows` ADR-010](https://github.com/flungo/github-workflows/blob/main/docs/decisions/010-caller-job-ids-match-the-workflow-filename.md): a caller's job id is the reusable workflow's filename). Not merely "contains Markdown": every repo does, and the flag is about the CI rather than the content. Both effects follow from adopting the standards — the lychee link-checker's token (`LYCHEE_GITHUB_TOKEN`) the external sweep reads is attached, and the `markdown-lint / lint` and `markdown-links / internal` checks the two workflows report are required. The sweep's own `markdown-links / external` is deliberately **not** required: it self-skips on `pull_request` and reports through an auto-updated issue, so that a third-party outage cannot block a merge. **Defaults `true`**, because every repo should end up here — so the flag's *absence* is the standard case and only its negation needs explaining. Set `markdown = false`, with a comment saying what adopting the standards would need, on a repo that has not adopted them; that includes any repo being created, which has no caller workflows yet (see the [creation runbook](../runbooks/creating-repositories.md)). A repo that has adopted them under non-conforming job names is not a `false` case — the fix there is to rename the caller jobs, not to reconcile the contexts. See [ADR-012](../decisions/012-markdown-flag-means-markdown-standards.md). |
...
| `shared_secrets` | `object` (sensitive) | `null` | The owner's shared secret values (`lychee_github_token`, `hcp_token` — both optional, since each is gated on its standards flag), composed once at owner level in a `locals` block and passed to every call as `shared_secrets = local.shared_secrets`. Required unless `manage_secrets = false`. |
