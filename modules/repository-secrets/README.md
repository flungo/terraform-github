...
Attaches the fleet's shared/common **GitHub Actions secrets** to a repository (see [ADR-005](../../docs/decisions/005-shared-secrets-module.md)).
This is the per-repository case; org-level shared secrets (`github_actions_organization_secret`) will be a separate `org-secrets` module when the first organisation is onboarded — the two are not symmetric (a personal account has no org-level secret), which is why shared secrets are their own module rather than folded into `repository`.
...
In CI they arrive via the reusable workflow's `tf_secret_vars` secret (composed from the owner's own Actions secrets with `toJSON`), exported as masked `TF_VAR_*` env vars; the variables are `sensitive`, so plan output and the plan artifact redact them.
Terraform-managing these secrets is safe because they are written to *other* repos — unlike the provider/HCP tokens that gate this repo's own CI, which stay manual (see [`docs/reference/secrets.md`](../../docs/reference/secrets.md)).
...
Referencing the repository module's `name` output makes the secrets depend on the repository — the repo is managed before its secrets are attached.
